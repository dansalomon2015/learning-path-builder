...
import { LearningPlan, StudyMode } from '../types';
...
interface DashboardProps {
  plans: LearningPlan[];
  onStartStudy: (plan: LearningPlan, mode: StudyMode) => void;
  onCreatePlan: (title: string, level: string) => void;
  onUpdatePlan: (planId: string, newTitle: string, newLevel: string) => void;
      ...
      <div>
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={() => onEdit(plan)} className="text-slate-400 hover:text-indigo-600 transition-colors p-1 rounded-full" aria-label={`Edit ${plan.title}`}>
              <PencilIcon className="w-5 h-5" />
            </button>
            <button onClick={() => onDelete(plan)} className="text-slate-400 hover:text-red-600 transition-colors p-1 rounded-full" aria-label={`Delete ${plan.title}`}>
        ...
        <p className="text-sm text-slate-500 mb-4">{plan.flashcards.length} flashcards</p>
      </div>
      <div className="relative mt-4" ref={dropdownRef}>
        ...
        {isDropdownOpen && (
          <div className="absolute bottom-full mb-2 w-full bg-white rounded-lg shadow-xl border border-slate-100 animate-fade-in p-2 z-10">
            <button
              onClick={() => {
                onStartStudy(plan, 'spaced-repetition');
                setIsDropdownOpen(false);
              }}
...
const Dashboard: React.FC<DashboardProps> = ({ plans, onStartStudy, onCreatePlan, onUpdatePlan, onDeletePlan }) => {
  ...
  const handleUpdateAndClose = (title: string, level: string) => {
    if (planToEdit) {
        onUpdatePlan(planToEdit.id, title, level);
    }
    setPlanToEdit(null);
  };
      ...
      {filteredPlans.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPlans.map((plan) => (
            <PlanCard key={plan.id} plan={plan} onStartStudy={onStartStudy} onEdit={setPlanToEdit} onDelete={setPlanToDelete} />
          ))}
        </div>
      ) : (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm">
            {searchQuery ? (
                 <>
                    ...
                    <p className="text-slate-500 mt-2">No learning plans match your search. Try different keywords.</p>
                </>
            ) : (
                <>
                    <h2 className="text-xl font-semibold text-slate-700">No learning plans yet.</h2>
                    <p className="text-slate-500 mt-2 mb-6">Click "Create New Plan" to start your learning journey!</p>
                    <button
                      onClick={() => setIsCreateModalOpen(true)}
