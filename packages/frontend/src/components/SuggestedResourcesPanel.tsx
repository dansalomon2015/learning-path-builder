...
const SuggestedResourcesPanel: React.FC<SuggestedResourcesPanelProps> = ({ resources }) => {
  ...
  // Trier par priority (1 = haute priorité)
  const sortedResources = [...resources].sort((a, b) => a.priority - b.priority);

  // Grouper par priority
  const highPriority = sortedResources.filter(r => r.priority <= 2);
  const recommended = sortedResources.filter(r => r.priority === 3);
  const optional = sortedResources.filter(r => r.priority >= 4);

  const getResourceIcon = (type: SuggestedResource['type']) => {
    switch (type) {
      case 'documentation':
  ...
  const renderResourceGroup = (group: SuggestedResource[], label: string, colorClass: string) => {
