...
const SpacedRepetitionMode: React.FC<{
  ...
  elapsedTime: number;
  onComplete: () => void;
}> = ({ plan, onBack, elapsedTime, onComplete }) => {
  ...
  const progressPercentage = useMemo(
    () => ((currentIndex + 1) / plan.flashcards.length) * 100,
        ...
        >
          ...
          <div className="absolute w-full h-full bg-indigo-500 text-white rounded-2xl shadow-xl flex items-center justify-center p-6 text-center [transform:rotateY(180deg)] [backface-visibility:hidden] cursor-pointer">
            <div>
              ...
              {card.explanation && <p className="text-sm opacity-90">{card.explanation}</p>}
            </div>
          </div>
        </div>
      </div>
...
const QuizMode: React.FC<{
  ...
  const quizOptions = useMemo(() => {
    ...
    const distractors = shuffleArray(
      plan.flashcards.filter(fc => fc.id !== currentCard.id).map(fc => fc.answer)
    ).slice(0, 3);
